...
class TRM_LSTMCell(RNNCellBase):
    def __init__(self, input_size, hidden_size, grid_size, bias=True, incell = True):
        super(TRM_LSTMCell, self).__init__()
        self.incell = incell
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.grid_size = grid_size
    ...
    def forward(self, input, hx):
        return self.trm_lstm_cell(
            input, hx,
            self.weight_ih, self.weight_hh,
            self.bias_ih, self.bias_hh)

    def trm_lstm_cell(self, input_a, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
        # self.spatial_embedding = torch.ones(self.spatial_embedding.size()).cuda()
        input = input_a[:,:2]
        #grid_input = input_a[:,-2:].type(torch.LongTensor).cuda() + config.spatial_width
        ...
        ingate, forgetgate, cellgate, outgate, memorygate = gates.chunk(5, 1)

        ingate = torch.sigmoid(ingate)
        forgetgate = torch.sigmoid(forgetgate)
        cellgate = torch.tanh(cellgate)
        outgate = torch.sigmoid(outgate)
        memorygate = torch.sigmoid(memorygate)
        cy_h = (forgetgate * cx) + (ingate * cellgate) + 0.1 * (memorygate * mx)
        # print 'cy size: {}'.format(cy_h.size())
        #cy_hh  = cy_h.data
        #cs = self.spatial_embedding.find_nearby_grids(grid_input)
        #atten_cs, attn_weights = self.atten(cy_hh,cs)
