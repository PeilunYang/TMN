...
coord_path='/home/peyang/Data/TrajSimilarity/NeuTraj/features/porto_traj_coord'
...
for i in range(0, 2000):
            ...
            if p < ((traj_len/10) + 1) :
                for q in range(15):
                    if q < ((j_traj_len/10) + 1):
                        # tmp = tdist.discret_frechet(np_traj_coord[i][0: min((p+1)*10, traj_len)], np_traj_coord[j][0: min((q+1)*10, j_traj_len)])
    ...
    if (i+1) % 200 == 0:
        #cPickle.dump(np.array(all_training_subtraj_similarity[((i+1)/200 - 1)*200: ((i+1)/200)*200]), open('/home/peyang/Data/TrajSimilarity/NeuTraj/features/subtraj_distance/training_' + str((i+1)/200 - 1) +'_dtw_distance','w'))
        #cPickle.dump(np.array(all_training_subtraj_similarity[0: 200]), open('/home/peyang/Data/TrajSimilarity/NeuTraj/features/subtraj_distance/training_' + str((i+1)/200 - 1) +'_dtw_distance','w'))
        np.save(open('/home/peyang/Data/TrajSimilarity/NeuTraj/features/geolife_subtraj_distance/training_' + str((i+1)/200 - 1) + '_erp_39_115_distance','w'), np.array(all_training_subtraj_similarity[0: 200]))
        # np.save(open('/home/peyang/Data/TrajSimilarity/NeuTraj/features/geolife_subtraj_distance/train_' + str((i+1)/200 - 1) + '_dtw_distance.npy', 'w'), np.array(all_training_subtraj_similarity[0: 200]))
        all_training_subtraj_similarity = []
